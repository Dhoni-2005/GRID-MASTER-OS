...
Phase 4 upgrade path:
    _select_task() is the scheduling extension point.
    Replace its body with weighted/priority scheduling without
    changing its signature or any other function.

Communication:
    Coordinator imports database, worker, reviewer only.
    ...
    No agent_registry, grid_master, or node_registry imports.
...
def _select_task(project_id: int) -> dict | None:
    ...
    Phase 3: returns the highest-priority task whose status is
    in _DISPATCHABLE, ordered by priority DESC then created_at ASC.

    Phase 4 upgrade:
        Replace this body with weighted scheduling, node-affinity
        routing, or priority queuing — without changing the signature.
        Caller always receives a task dict or None.
    """
    ...
    # Sort: highest priority first; earliest created_at as tiebreaker
    candidates.sort(
...
def _dispatch(task:       dict,
    ...
    if status == "planned":
        return _dispatch_to_worker(task_id, project_id, node_id, timeout)
...
def _dispatch_to_worker(task_id:    int,
                        project_id: int,
                        node_id:    str | None,
                        timeout:    int) -> dict:
    """
...
if __name__ == "__main__":
    ...
    for mod in ["database", "memory_manager", "worker", "reviewer", "coordinator"]:
